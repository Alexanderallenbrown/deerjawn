...
def Brakes(vehicleID, controller,obj,mu):
    ...
    try:
        ...
        #print(joy.axisValues)
        val = 32767-joy.axisValues[3]

        front_Brake = val*brakeAmount/(2*32768.0)
        back_Brake = val*brakeAmount/(2*32768.0)
        if front_Brake>Fbmax:
            front_Brake=Fbmax
        if back_Brake>Fbmax:
            back_Brake=Fbmax
        brakes = True
        #print ('brakes: '+str(front_Brake))
    except:
        #print("using keyboard")
        brake = controller.sensors["Brake"]
        gas = controller.sensors["Gas"]
        if brake.positive==True:
            front_Brake = brakeAmount/32768.0
            back_Brake = brakeAmount/32768.0
            if front_Brake>Fbmax:
                front_Brake=Fbmax
            if back_Brake>Fbmax:
                back_Brake=Fbmax
...
def Power( vehicleID, controller, brakes,obj,mu):  
    ...
    try:
        joy = controller.sensors["joygas"]
        #print(joy.axisValues)
        val = 32767-joy.axisValues[2]
        power = -val/(2*32768.0)*gasPower
        #print("power: "+str(power))
    except:
        #print("using keyboard")
        gas = controller.sensors["Gas"]
        if gas.positive==True:
            power = -32768.0*gasPower
        else:
            power = 0
        #print("power: "+str(power))
    # # brakes
...
def Steering( vehicleID, controller):
    try:
        ...
        #print("using keyboard")
        steerleft = controller.sensors["SteerLeft"]
        steerright = controller.sensors["SteerRight"]
        # set turn amount
        if steerleft.positive==True:
            # get steering sensors
            turn = -1*20.0/26000.0*3.14/180.0 
            #print("steer = "+str(turn))
            #else:
             #   pass
                #turn = 0.0
